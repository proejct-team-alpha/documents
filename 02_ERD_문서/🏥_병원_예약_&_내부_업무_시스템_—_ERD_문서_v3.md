# 🏥 병원 예약 & 내부 업무 시스템 — ERD 문서 v3

> **문서 버전:** v3.0 (v4.1 계획서 기반 재구성)
**작성일:** 2026년
**연관 문서:** 프로젝트 계획서 v4.1 / 화면 흐름 시퀀스 다이어그램
**변경 내역:** CHATBOT_HISTORY 테이블 추가, RESERVATION.source 컬럼 추가, ITEM.category 한글 레이블 통일
>
...
5. [v2.0 → v3.0 변경 내역](#5-v20--v30-변경-내역)
...
| 항목 | ERD v2.0 | ERD v3.0 | 변경 이유 |
| --- | --- | --- | --- |
| 테이블 수 | 9개 | **10개** | CHATBOT_HISTORY 추가 |
| CHATBOT_HISTORY | 없음 | **신규 추가** | 챗봇 대화 이력 세션 단위 관리 |
| RESERVATION.source | 없음 | **컬럼 추가** | 예약 경로(ONLINE/PHONE/WALKIN) 구분 필요 |
| ITEM.category 한글 레이블 | `GENERAL_SUPPLIES` = 일반 비품 | `GENERAL_SUPPLIES` = **일반 소모품** | 한글 레이블 통일 |
| CHATBOT_HISTORY FK | — | **STAFF.id 참조** | 직원별 대화 이력 관리 |
