...
> **문서 버전:** v4.0
> **작성일:** 2026년
> **변경 이력:** v2.1 (3개 분권, 61개) → v3.0 (단일 문서, 컨트롤러 반환 패턴 전면 개정) → **v4.0 (PBL 요구사항 대응 — /api/** JSON API 레이어 추가, @Valid + BindingResult 유효성 검증 패턴 추가, 진료과 activate/deactivate API 정합화, available_days 검증 로직 명시)**
...
| `POST` | `/api/reservations/{id}/cancel` | 예약 취소 | ROLE_STAFF, ROLE_ADMIN |
...
| # | 메서드 | URL | 반환 | 설명 |
|---|--------|-----|------|------|
| 63 | POST | `/api/staff/{id}/update` | JSON | 직원 정보 수정 |
| 64 | POST | `/api/patients/{id}/update` | JSON | 환자 정보 수정 |
| 65 | POST | `/api/reservations/{id}/cancel` | JSON | 예약 취소 |
| 66 | POST | `/api/items/{id}/delete` | JSON | 물품 삭제 |
| 67 | POST | `/api/rules/{id}/delete` | JSON | 규칙 삭제 |
