...
> **기본 철학:** `develop` 브랜치가 개발의 중심. `main`은 배포/릴리스 전용.

> **변경 이력**
> | 버전 | 변경 내용 |
> |---|---|
> | v1.0 | 초안 작성 |
> | v2.0 | 개발 브랜치명 `dev` → `develop` 통일 (아키텍처/계획서와 정합) |
...
- **`develop` 브랜치** : 모든 개발 통합 브랜치 (직접 푸시 금지 권장)
- 모든 변경은 **`feature/` `bugfix/` `hotfix/`** 등 브랜치 → PR → `develop` 병합
- 코드 리뷰 **최소 1명 이상 Approve** 필수
- conflict 발생 시 **본인이 직접 해결**
- `develop` 브랜치 보호 설정 추천 (GitHub Settings → Branches → Branch protection rules)
...
```bash
git clone -b develop --single-branch https://github.com/팀명/프로젝트명.git
cd 프로젝트명

# (선택) 기본 브랜치를 develop으로 설정해두면 편함
git remote set-head origin develop
```

이후에는 작업 시작 전 항상 `develop`를 최신화합니다.
...
git pull --ff-only origin develop    # fast-forward만 허용 (충돌 시 알려줌)
# 또는 그냥
...
- 최소 1명 Approve 후 **Squash and merge**로 `develop`에 병합 (기본값 추천)

### Step 4. 다른 팀원의 변경이 develop에 merge된 후 — 본인 브랜치 업데이트

브랜치로 돌아와서 `develop`를 가져옵니다. 두 가지 방법 중 선택 (팀 컨벤션으로 하나 정하는 걸 추천)
...
git fetch origin
git merge origin/develop          # 또는 git pull origin develop --no-rebase
...
|---|---|---|---|---|
| `feature/*` | `develop` | `develop` | 1~7일 | merge 후 |
| `bugfix/*` | `develop` | `develop` | 1~5일 | merge 후 |
| `hotfix/*` | `main` | `main` + `develop` | 1~3일 | merge 후 |
| `release/*` | `develop` | `main` | 배포 기간 동안 | 배포 완료 후 |
...
- `develop` 브랜치 보호 설정 추천 (Require PR, Require 1 approve, Restrict who can push)
- force push는 `--force-with-lease`만 허용, 일반 `--force` 금지
- 매일 아침 `develop pull` 습관화 (슬랙/팀 채팅에 "develop pull 완료" 정도만 공유해도 OK)
